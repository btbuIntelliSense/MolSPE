...
from Datamodel.data_utils import MotifAggregator, AtomBondEncoder
from EMPP.graph_attention_transformer import GraphAttentionTransformer

from MaskModel.gnn_model import GNN
...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        mode=None,
        motif_vocab_size = 300,  # 新增motif字典大小
        motif_embed_dim = 128,  # motif嵌入维度
        # use_dataset_md17=True,
        use_empp = True,  # 新增：是否使用EMPP
        empp_num_mask = 1,  # 每个分子掩码的原子数
        empp_resolution = 100,  # 球面网格分辨率
    ):
        ...
        # print(self.max_z)  128
        # 添加EMPP相关参数
        self.use_empp = use_empp
        if self.use_empp:
            self.empp_head = GraphAttentionTransformer(
        irreps_in=None,
        ...
        mean=None, std=None, scale=None, atomref=None, ssp=True)
    ...
    def forward(self, z, pos,n_atom,mask_idx,motif_atoms,batch):

        x = self.embedding(z) #(92,64) # 原子序数
        ...
        xnew = self.out_norm(x) #2381,256
        if self.layernorm_on_vec:
            vec = self.out_norm_vec(vec) #(92,3,64)

        empp_loss=0
        if self.use_empp:
            # 计算EMPP损失
            empp_loss = self.empp_head(pos, batch, z, n_atom, mask_idx,motif_atoms,target=None, ssp=True,motif=True)
            # motif_empp_loss=self.compute_motif_empp_loss(z, pos, batch, n_atom, motif_atoms)

        return xnew, vec, z, pos, batch, empp_loss
...
class EquivariantMultiHeadAttention(MessagePassing):
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij): #x, vec, edge_index, edge_weight(pos), edge_attr, edge_vec
        x = self.layernorm(x)
