...
class TransformerEncoderWithPair(nn.Module):
    ...
    def forward(
        ...
        bsz = emb.size(0)
        seq_len = emb.size(1)
        x = self.emb_layer_norm(emb)  #原子特征
        # edge_attr=self.emb_layer_norm(edge_attr)
        x = F.dropout(x, p=self.emb_dropout, training=self.training)
        # edge_attr=F.dropout(edge_attr, p=self.emb_dropout, training=self.training)
        # account for padding while computing the representation
        if padding_mask is not None:
            x = x * (1 - padding_mask.unsqueeze(-1).type_as(x))
            # edge_attr = edge_attr * (1 - padding_mask.unsqueeze(-1).type_as(edge_attr))
        ...
        for i in range(len(self.layers)):
            ...
            edge_attr, attn_mask_edge, _ = self.layers[i](
                    edge_attr, padding_mask=padding_mask, attn_bias=attn_mask, return_attn=True
                )
        ...
        return x, attn_mask, delta_pair_repr, x_norm, delta_pair_repr_norm,edge_attr
