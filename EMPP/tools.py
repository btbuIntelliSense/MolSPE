...
class ToS2Grid_block:
    ...
    def ToGrid(self, x):
        x_grid = torch.einsum("mbi,zci->zcbm", self.to_grid_shb.to(device=x.device), x)
        x_grid = torch.einsum("am,zcbm->zcba", self.to_grid_sha.to(device=x.device), x_grid).contiguous()
        return x_grid