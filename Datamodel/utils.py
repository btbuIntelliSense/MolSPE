...
def make_splits(
    ...
    # 计算实际样本数量
    train_samples = int(dataset_len * train_size)
    val_samples = int(dataset_len * val_size)
    test_samples = dataset_len - train_samples - val_samples

    # 生成随机索引
    idx = np.random.RandomState(seed).permutation(dataset_len)
    return (
        torch.from_numpy(idx[:train_samples]),
        torch.from_numpy(idx[train_samples:train_samples+val_samples]),
        torch.from_numpy(idx[train_samples+val_samples:]),
    )
