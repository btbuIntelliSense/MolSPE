...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        return self.propagate(edge_index[0], x=x, edge_attr=edge_embeddings) #,edge_embeddings
...
class GATConv(MessagePassing):
    def __init__(self,
        ...
        self.edge_embedding1 = torch.nn.Embedding(num_bond_type, emb_dim)
        # self.edge_embedding2 = torch.nn.Embedding(num_bond_direction, emb_dim)
        self.edge_update_layer = nn.Linear(emb_dim, emb_dim)  # 新增边更新层
        torch.nn.init.xavier_uniform_(self.edge_embedding1.weight.data)
        # torch.nn.init.xavier_uniform_(self.edge_embedding2.weight.data)
        ...
        self.edge_update1 = Parameter(torch.Tensor(emb_dim + self.edge_dim, emb_dim))   # [emb(out)+edge_dim] x emb(out)  # new
    ...
    def forward(self, x, edge_index, edge_attr, size=None):
        ...
        edge_attr = self.edge_embedding1(edge_attr)
        edge_attr = self.edge_update_layer(edge_attr)
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin"):
        ...
        torch.nn.init.xavier_uniform_(self.x_embedding1.weight.data)   #Xavier 均匀初始化方法，权重矩阵的每个元素将从均匀分布中采样
    ...
    def forward(self, *argv):
        ...
        # 获取最后一层卷积的边特征
        last_gat_layer = self.gnns[-1]
        edge_rep = last_gat_layer.get_edge_attr()
        return node_representation,edge_rep
